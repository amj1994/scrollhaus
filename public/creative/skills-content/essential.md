# TypeUI · Essential — Design System
...
Essential is a **polished, production-grade TypeUI design system** expressed as **stack-agnostic design tokens**. Its character is bright, soft, and tactile: **neutral light-grey surfaces** under dark `#1A1A1A` headings and `#4D4D4D` body, a single **vivid blue accent** (`#256EFF`) for CTAs, highlights, and focus, **pill controls on generously rounded 20px cards**, and **real soft depth — surfaces lift on layered shadows**, not hairline borders, with a glossy sheen on filled buttons. The result should feel *bright and pressable* — cards float on a gentle cushion, buttons look tactile. If a screen built from these modules reads as flat, sharp-edged, or off-brand (e.g. still orange/warm), it isn't Essential yet.

### Signature traits — non-negotiable

These are what make a screen recognizably Essential. Hold every one of them, on every surface:

- **Pill controls on 20px cards** — buttons, badges, and chips round to a full pill (`radius-full`); cards, modals, menus, tabs, tables rest at `radius-xxl` (20px); inputs at `radius-lg` (12px). This is the defining shape. See `radius.md`.
- **Pill controls share `radius-full`** (999px) — buttons, badges, chips, plus the toggle track, avatars, radio, range, status dots; only cards and panels step to the 20px shell.
- **One typeface — Inter everywhere** — every surface (marketing, editorial, and product UI) is set in **Inter**; `font-family` and `font-family-heading` both resolve to Inter, so headings, titles, body, labels, and controls share the one grotesk. No serif, no second display face. Compact 14px (`font-size-sm`) baseline under a disciplined heading ramp. See `typography.md`.
- **One marketing surface — `#F5F5F5`, no exceptions** — every marketing section (hero, content bands, footer) is the same `#F5F5F5` canvas; sections are separated by the embossed `#E0E0E0` hairline frame, not by a change of fill. The blue brand (`#256EFF`) and dark (`#1A1A1A`) are never section backgrounds. Dashboard/application UI is a separate white-chrome context. See `sections.md` and `colors.md`.
- **Soft layered depth** — cards, controls, and overlays lift off the page on **stacked soft shadows** (`elevation-*`), and filled pill buttons add the glossy `shadow-inset-gloss` sheen. Depth and roundness carry hierarchy, not hairline borders. See `shadows.md`.
- **Clean rounded cards on a soft shadow** — cards fill white/light, round to `radius-xxl` (20px), and lift on `elevation-2` (rising to `elevation-3` on hover) — no film grain, no border-only edge. This soft, floating panel is the signature card. See `cards.md`.
...
|---|---|
| Landing / marketing page | foundation + `sections.md`, `film-grain.md`, `buttons.md`, `cards.md`, `alerts.md` |
...
- **Tokens are the vocabulary, and they are agnostic.** `neutral-primary-soft`, `heading`, `default-medium`, `spacing-4`, `radius-xxl`, `elevation-1` are Essential tokens, not framework utilities — map them into your stack's token layer.
- **Hold the signature radius.** Shells are `radius-xxl` (12px, rounded); only functionally round controls (toggle track, avatars, radio, range) use `radius-full`. Shipping sharp 0px or pill corners quietly breaks the theme.
- **Bordered and flat.** No resting card or component wears a drop shadow (floating overlays — dropdowns, menus, popovers — are the one documented exception and carry a real medium shadow; see `dropdowns.md`). Marketing/editorial cards and widgets carry the warm `surface-card` (`#FFFFFF`) fill with a film-grain texture inside a hairline `default` (`#E0E0E0`) border — the same on every section, never matched to the band and never a shadow. Inputs and every other control (select, textarea, checkbox, radio, toggle) match the background of the surface they sit on and are defined by a `default` (`#E0E0E0`) border — never a contrasting fill. Outline buttons carry a 2px border. Classic, solid buttons (see `buttons.md`).
- **Application dashboard surfaces are white.** On dashboard/application routes, **every widget, sidebar, navbar, and card uses a `#FFFFFF` (`neutral-secondary-soft`) background** — separated by the hairline `default` (`#E0E0E0`) border and spacing alone, never `surface-card`, a tint, or a shadow. The warm `surface-card` fill is the marketing/editorial card look only. See `sections.md` and `cards.md`.
- **Section surface rhythm.** Marketing: **one `#F5F5F5` surface on every band** (hero, content, footer) — no white/dark alternation, no brand-colored bands; sections are separated by the embossed `#E0E0E0` hairline frame and are **square-edged (never rounded)**. Dashboard/application: **white only** — no brand bands, no dark bands, no section rounding. **Marketing section padding:** `96px` top and bottom on large screens (`≥ 1024px`); dashboard exempt. See `sections.md`.
- **Equal vertical breathing room between sections.** Every marketing section carries the **same spacing above and below it** — on large screens that is **96px** top and bottom per `sections.md`. Dashboard routes use app spacing instead.
...
- **No vendor leakage.** Describe and implement through Essential tokens; never paste third-party component class strings from external docs into the work.
...
- **Alert dismiss stays neutral on white cards.** A close (×) control is **never brand-colored** unless the **alert root itself** is an intent-tinted banner (`brand-softer`, `success-soft`, etc.). White dashboard cards, sidebar promo cards, and widgets with a tinted status icon still use **neutral dismiss** (`body-subtle` → `heading`, `neutral-tertiary` hover). Never reuse `btn btn--ghost btn--icon` for dismiss. See `alert-dismiss-controls.md`.
...
- [sections.md](sections.md) — one `#F5F5F5` marketing surface + the embossed hairline frame, square-edged sections, dashboard white-only rule
- [film-grain.md](film-grain.md) — static film-grain texture in the side gutters (outside 1280px) on every landing page; never over content, never on dashboard
...
- [radius.md](radius.md) — border-radius scale (`radius-xs` … `radius-full`) and the 12px rounded shell convention
...
- [card-corner-lines.md](card-corner-lines.md) — decorative bottom-right corner hatch on marketing cards (not divided grid cells, never dashboard/application widget cards)
- [alerts.md](alerts.md) — inline feedback messages (success, error, warning, info)
- [alert-dismiss-controls.md](alert-dismiss-controls.md) — close (×) controls: neutral on white cards, never ghost buttons
