...
**Style: Bold** — a confident, high-impact aesthetic driven by oversized ultra-heavy headings, generous whitespace, and a punchy green brand (#3EAA49) over warm cream backgrounds. Headings use the display sans-serif Archivo Black for maximum typographic weight; body text and UI labels use Inter at medium weight (500) for readability. Subtle hairline borders (1–2px), soft layered shadows, and generously rounded surfaces (12px base) keep every element warm, grounded, and inviting. The result is a modern, editorial-grade design where bold typography commands a warm, organic page.
...
| Context | Width |
|---|---|
| Structural Grid (sections, columns) | 1px (`rgba(20, 32, 30, 0.08)`) |
...
| Cards / containers | 1px; avoid stacked heavy borders |
...
| Token | Light | Dark |
|---|---|---|
| neutral-primary-soft | #FFFFFF | #1A2A26 |
| neutral-primary | #FCFBF8 | #11201C |
| neutral-primary-medium | #FCFBF8 | #243733 |
| neutral-primary-strong | #FFFFFF | #34463F |
| neutral-secondary-soft | #FAF7F0 | #1A2A26 |
| neutral-secondary | #F8F5EB | #11201C |
| neutral-secondary-medium | #F2EACF | #243733 |
| neutral-secondary-strong | #F8F5EB | #34463F |
| neutral-tertiary-soft | #F1E8CA | #1A2A26 |
| neutral-tertiary | #F1E8CA | #243733 |
| neutral-tertiary-medium | #ECE2BC | #34463F |
| neutral-quaternary | #E3D8AE | #34463F |
| quaternary-medium | #E3D8AE | #4A5C54 |
| gray | #CFC6A6 | #4A5C54 |

### Brand
| Token | Light | Dark |
|---|---|---|
| brand-softer | #E7F3E1 | #E7F3E1 |
| brand-soft | #B4DDAA | #B4DDAA |
| brand | #3EAA49 | #3EAA49 |
| brand-medium | #348F3D | #348F3D |
| brand-strong | #2C7A34 | #2C7A34 |
...
|---|---|---|
| dark | #14201E | #14201E |
| dark-strong | #0E1816 | #243733 |
| disabled | #EFE9D8 | #14201E |
...
| white | #FFFFFF | #FFFFFF |
| black | #14201E | #14201E |
| heading | #14201E | #FCFBF8 |
| body | #57513E | #C7BFA8 |
| body-subtle | #847C66 | #A39B83 |

### Brand
| Token | Light | Dark |
|---|---|---|
| fg-brand-subtle | #8FCB86 | #8FCB86 |
| fg-brand | #3EAA49 | #5FBF69 |
| fg-brand-strong | #2C7A34 | #7FD089 |
...
| Token | Light | Dark |
|---|---|---|
| border-dark | #14201E | #4A5C54 |
| border-buffer | #FFFFFF | #11201C |
| border-buffer-medium | #FFFFFF | #14201E |
| border-buffer-strong | #FFFFFF | #243733 |
| border-muted | #F5F0E2 | #11201C |
| border-light-subtle | #EFE7D2 | #11201C |
| border-light | #EDE4CC | #14201E |
| border-light-medium | #E8DEC2 | #243733 |
| border-default-subtle | #E5DCC0 | #11201C |
| border-default | #E0D6B8 | #14201E |
| border-default-medium | #DACEAB | #243733 |
| border-default-strong | #CFC2A0 | #4A5C54 |
...
| border-brand-subtle | #B4DDAA | #B4DDAA |
| border-brand-light | #3EAA49 | #3EAA49 |
| border-brand | #3EAA49 | #3EAA49 |
| border-dark-subtle | #14201E | #243733 |
| border-purple | #A855F7 | #A855F7 |
| border-orange | #FEC76C | #FEC76C |
...
Each section has:
- 96px vertical padding, applied **equally on top and bottom** — every section's top spacing must equal its bottom spacing, so the gap between any two adjacent sections is symmetric. No section gets more air on one side than the other.
- A warm, light background. Alternate between `neutral-primary` (warm white) and `neutral-secondary-soft` (warm cream) down the page to create a calm, inviting rhythm. Keep the palette warm — never go dark/immersive for standard content sections.
...
The layout relies on a subtle 1px structural grid.
- Use `1px solid rgba(20, 32, 30, 0.08)` (or equivalent dark mode border) to separate columns, rows, and sections.
- Sections should often use CSS Grid (`grid-template-columns`) with border-right on columns and border-bottom on rows to create a blueprint-like structure.

## Technical Background

The main canvas should feature a faint technical grid over the warm background:
```css
background-image: linear-gradient(rgba(20, 32, 30, 0.035) 1px, transparent 1px), linear-gradient(90deg, rgba(20, 32, 30, 0.035) 1px, transparent 1px);
background-size: 100px 100px;
```

## Backgrounds & Visual Depth

- Default to flat, warm solid-color fills — soft elevation and warm tint shifts are the primary depth tools.
- Apply contextual treatments — soft layered shadows, subtle hairline borders, warm color blocks, generous rounded surfaces — that align with the warm, bold-green aesthetic.
...
- All sections: consistent 96px vertical padding, equal on top and bottom — the spacing between every pair of adjacent sections must be symmetric
...
| Token | CSS value |
|---|---|
| shadow-2xs | `0 1px 2px rgb(20 32 30 / 0.05)` |
| shadow-xs | `0 1px 3px rgb(20 32 30 / 0.08)` |
| shadow-sm | `0 2px 6px rgb(20 32 30 / 0.08)` |
| shadow-md | `0 4px 12px rgb(20 32 30 / 0.10)` |
| shadow-lg | `0 8px 20px rgb(20 32 30 / 0.12)` |
| shadow-xl | `0 16px 32px rgb(20 32 30 / 0.14)` |
| shadow-2xl | `0 24px 48px rgb(20 32 30 / 0.18)` |
...
- **Heading font:** Archivo Black, sans-serif — configured at app level, never override
- **Body font:** Inter, sans-serif — regular weight (400) baseline — configured at app level, never override
- **Headings:** regular weight (400), heading text color — Archivo Black is inherently ultra-heavy, do not add bold
- **Body copy:** regular weight (400), body text color, never use brand color for paragraphs longer than one sentence
...
Apply `overflow-wrap: break-word` and `word-break: break-word` to headings when copy may overflow narrow columns.
...
Use "Micro Labels" for metadata, navigation, and structural markers to create contrast against headings.
- Size: 10px or 11px
- Transform: uppercase
...
The homepage uses **headings only** — no paragraph text. Every piece of copy on the homepage must be rendered as a heading element (`h1`–`h6`). Body paragraphs, leading paragraphs, and supporting copy are reserved for interior/detail pages, never the homepage.
