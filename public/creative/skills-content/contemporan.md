# TypeUI · Contemporan — Design System
...
This skill is the **authoritative visual specification for TypeUI Contemporan** (from [typeui.sh](https://www.typeui.sh)). Everything you build for this project — every component, layout, and page — follows the module files bundled here. They define *what Contemporan looks like* down to the token; you decide how to implement it (plain CSS, utility classes, CSS-in-JS, design tokens — any styling approach), but you implement this design — you do not redesign it.
...
Contemporan is a **polished, production-grade TypeUI design system** expressed as **stack-agnostic design tokens**. Its character is bold, technical, and confident: a saturated **electric** ultramarine brand (`#0000F2`) reserved for the **hero and footer** while the body of the page is **white sections divided by hairline borders**, a bright **lime accent** (`#EDFF45`), **hairline 1px corners**, **white bordered cards**, and **borderless non-card components that invert against their surface** (brand on white, white on brand) — no shadows. The result should feel *clean, electric, and precise* — terminal-meets-editorial, never noisy, never floating. If a screen built from these modules reads as soft-cornered, shadowed, or warm-toned, it isn't Contemporan yet.

### Signature traits — non-negotiable

These are what make a screen recognizably Contemporan. Hold every one of them, on every surface:

- **1px (hairline) corners on every component shell** (`radius-xxl`) — buttons, inputs, cards, modals, menus, alerts, tabs, tables, tooltips, badges. **Every component carries only this 1px corner**; soft 4px+ or pill control corners are a different theme. See `radius.md`.
- **Functionally round controls only** (`radius-full`, 999px) — the toggle track, avatars, radio, range, status dots; everything box-shaped keeps the hairline 1px shell.
- **Serif marketing headings, monospace everywhere else** — marketing & editorial headings/display set in the high-contrast display serif **Playfair Display** (`font-family-heading`). **Dashboard & application chrome — page titles, widgets, sidebars, topbars, tables — uses the monospace Geist Mono** (`font-family`), like body, labels, and controls. Compact 14px (`font-size-sm`) even-width mono baseline under a disciplined serif heading ramp; the serif is reserved for marketing/landing/editorial surfaces. See `typography.md`.
- **White sections, full-bleed blue, brand hero/footer (marketing/ecommerce)** — sections use **only white (`#FFFFFF`) or blue (`brand` `#0000F2`)**. The **blue is full-bleed / full-width** (page background, hero, footer span edge to edge); **white sections are inset `spacing-8` (32px) on the left and right** so a 32px blue gutter frames them (the reference look). **Every section is white except the hero and footer, which are blue.** Sections are square-edged, **with white sections divided from each other by a 1px `default` (`#DDE1F5`) border (white-to-white only — **no border at the hero/footer boundary**, space alone there)**, with a **uniform 96px gap** (48px below 1024px). Dashboard/application UI is **white only**. See `sections.md` and `colors.md`.
- **Cards are always white + bordered** — every card has a **`white` (`#FFFFFF`)** background on every surface (white section, blue section, dashboard) and, because it is white, **always carries a hairline `default` (`#DDE1F5`) border**. Cards never invert to a brand fill. See `cards.md`.
- **Form fields match surface + border; buttons/badges invert** — **inputs, selects, and textareas match the background they sit on** (`white` on a white section, the **same blue `#0000F2` on the hero/footer**) and **always carry a `default` (`#DDE1F5`) border** — never a contrasting fill. **Buttons and badges** carry no border and **invert** against their surface (white on blue, brand on white) — under a brand focus ring; inside a white card they take the brand fill. See `colors.md` and `input-field.md`.
- **Dashboard is white with bordered chrome** — dashboard/application routes are white throughout, so the **widgets, navbar, sidebars, and cards carry a hairline `default` (`#DDE1F5`) border** to read apart (no inversion). See `cards.md` and `sidebar.md`.
...
- **Tokens are the vocabulary, and they are agnostic.** `neutral-primary-soft`, `heading`, `default-medium`, `spacing-4`, `radius-xxl`, `elevation-1` are Contemporan tokens, not framework utilities — map them into your stack's token layer.
- **Hold the signature radius.** Every component shell is `radius-xxl` (1px); only functionally round controls (toggle track, avatars, radio, range) use `radius-full`. Shipping 4px+ or pill corners quietly breaks the theme.
- **Cards & form fields: matched fill + border; buttons/badges: borderless inversion.** **Cards are always `white` with a `default` (`#DDE1F5`) border.** **Form fields (inputs, selects, textareas) match the surface they sit on — `white` on a white section, the same `brand` blue on the hero/footer — and always carry a `default` border** (never a contrasting fill). **Buttons and badges** wear no border and **invert** against their surface (`white` on blue, `brand` on white; inside a white card they take the brand fill); buttons are solid or soft tonal fills, never outlined. **On the application dashboard** everything is white and widgets, navbar, sidebars, and cards carry the `default` border. See `colors.md`, `cards.md`, `input-field.md`, `buttons.md`.
- **Marketing layout: full-bleed blue, white sections inset 32px on the sides.** Sections are **white or blue only**. The **blue (`brand` `#0000F2`) is full-bleed / full-width** (page background, hero, footer); **white sections are inset `spacing-8` (32px) on the left and right** so a 32px blue gutter shows down both sides — white never touches the side edges. Every section is white except the brand hero and footer. Sections are square-edged, **with white sections divided from each other by a 1px `default` (`#DDE1F5`) border (white-to-white only — **no border at the hero/footer boundary**, space alone there)**; no dark band, no third surface. Dashboard/application: **white only** — no brand bands, no rounding. See `sections.md`.
- **Equal breathing room between sections.** Every section uses the **same vertical padding** (`96px` ≥1024px, `spacing-12`/48px below), so the **gap between any two sections is identical** — no two sit closer or farther apart than another — and a 1px `default` border divides one white section from the next (but **not** the hero→first-section or last-section→footer boundary, which is space only). Dashboard routes use app spacing instead.
...
- **No vendor leakage.** Describe and implement through Contemporan tokens; never paste third-party component class strings from external docs into the work.
...
- **Motion is quiet and consistent.** Interaction states (hover, focus, active; color / background / border changes) transition in **150ms**; larger positional or reveal motion (slide-in, expand, menu open, drawer) in **~200–300ms ease**; opacity fades in **150ms**. Honor `prefers-reduced-motion` by dropping movement and keeping the state change instant. No bounce, no spring, no decorative looping animation.
...
- [sections.md](sections.md) — marketing section surfaces (white, brand, dark), square-edged sections, dashboard white-only rule
...
- [radius.md](radius.md) — border-radius scale (`radius-xs` … `radius-full`) and the 1px crisp shell convention
...
- [sidebar.md](sidebar.md) — persistent dashboard navigation rail; nav link states derived from the sidebar surface
