...
## Style
A friendly, modern interface with clean lines, generous pill-shaped controls, soft rounded cards, vibrant pastel accents, and ample whitespace for a polished, approachable experience inspired by minimals.cc
...
All badges are pill-shaped (9999px) by default. No separate pill variant needed.
...
- A white (#FFFFFF) background — all sections share the same white background, no alternating colors
...
- The entire landing page / website has a single, continuous modern gradient applied at the page level — not repeated per section. Use a subtle warm-to-cool diagonal gradient (e.g. very soft peach to lavender at low opacity) that spans the full page height behind the white content.
- Individual sections do NOT get their own background colors or gradients — they are transparent over the page-level gradient.
- Keep the overall feel clean and airy with generous whitespace.
...
- Title: 20px, 700 weight, heading color
...
- **Font:** Public Sans, sans-serif — configured at app level, never override
