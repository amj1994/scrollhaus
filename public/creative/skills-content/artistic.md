...
**Style: Artistic** — an editorial, hand-crafted aesthetic that blends warm cream backgrounds, bold coral/terracotta accents, deep navy typography, hard offset shadows, thick visible borders, and playful illustrated depth. Headings use the Art Deco display face Limelight; body text and UI labels use the refined serif IBM Plex Serif. The result is a design that feels simultaneously artisanal and modern — like a beautifully printed magazine brought to the screen.
...
### Separated Cards
Each item is independent — has its own 2px border, 16px radius, and shadow-sm. 8px bottom margin between items. No shared outer border.
...
- **Body:** 14px, normal weight, 1.65 line-height
...
- Thick visible borders are a key part of the neobrutalism aesthetic
...
| Base (default) | 14px | 18px | 12px |
| Large | 16px | 22px | 14px |
| Extra large | 16px | 28px | 16px |

## Padding Rules

- **Always use the Sizes table above** — do not invent oversized padding (e.g. `px-10 py-5`, `px-16 py-6`, or heights above ~48px) unless Extra large is explicitly chosen.
- Hero, marketing, and dashboard CTAs use **Base** or **Large** from the scale — the same normal padding used across other design systems.
- Do not create custom one-off button dimensions in component code.
...
- Body copy: 16px, 1.75 line-height, IBM Plex Serif
...
- Rounded square: 16px radius (MD/LG/XL), 10px radius (XS/SM)
...
- **Background:** neutral-secondary-medium
- **Shadow:** shadow-xs
- **Font:** 14px, heading color, IBM Plex Serif
...
- Display: block
- Font: 14px, medium weight, heading color, IBM Plex Serif
- Margin bottom: 8px
...
- A background color (alternate between neutral-primary-soft and neutral-secondary-soft — warm cream tones)
...
- Neobrutalism favors snappy, confident transitions — keep durations short (150–250ms) with ease-out curves.

## Backgrounds & Visual Depth

- Default to warm cream layered backgrounds with editorial character rather than flat solid fills.
- Apply contextual treatments — hand-drawn textures, sketch-style patterns, sticker-like element borders, layered warm transparencies, hard offset shadows, decorative thick borders, paper grain overlays — that reinforce the artistic neobrutalism aesthetic.
...
- Illustrated editorial artwork (comic/sketch style) is encouraged for hero and feature sections.
...
See `typography.md` for the full heading scale. These layout-level rules override display marketing sizes on app surfaces:

### Dashboard pages

- Set `data-surface="dashboard"` on the root app shell.
- **All headings** (`h1`–`h6`) must stay compact: **maximum 28px, preferably 24px**.
- Never use landing-page display sizes in sidebars, tables, stats, or settings.

### E-commerce pages (app surfaces)

- Set `data-surface="ecommerce-app"` on the storefront layout root.
- **All headings outside the storefront hero** use the same compact rule as dashboard: **max 28px, prefer 24px**.

### E-commerce storefront hero (exception)

- Only the above-the-fold marketing hero may use the full landing heading scale (`h1` up to 60px desktop).
- Mark the hero with `data-section="storefront-hero"` inside `data-surface="ecommerce-app"`.
- Sections below the hero revert to compact e-commerce heading rules.
...
|---|---|---|
| base | 16px | Buttons, cards, inputs, modals, sections |
| default | 10px | Badges, tooltips, dropdown items, small controls |
| sm | 6px | Checkboxes, tiny elements |
| full | 9999px | Pills, avatars, toggles, dot indicators |

## Rules

- 16px is the default radius across the product
...
- **Heading font:** "Limelight", cursive — Art Deco display face, configured at app level via Google Fonts import, never override
- **Body font:** "IBM Plex Serif", serif — refined serif for all non-heading text, configured at app level via Google Fonts import, never override
- **Headings:** normal weight (400), heading text color — Limelight is a display-only weight font, do not apply bold/semibold
...
| Logo / wordmark (non-heading) | 22px | 400 |

All UI labels use IBM Plex Serif. Do not apply paragraph line-height (1.75) to control labels.
